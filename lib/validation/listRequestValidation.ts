...
const sortValidationRules = joi.object<Sort>({
    sortBy: joi.any().only().allow('updatedAt', 'name'),
    sortDirection: joi.any().only().allow('asc', 'desc'),
});

const filterValidationRules = joi.object<Filter>({
    difficulty: joi
        .array()
        .items(joi.any().only().allow('EASY', 'MEDIUM', 'HARD')),
    cookingTime: joi.number().integer().greater(0),
    prepTime: joi.number().integer().greater(0),
});

export const listRequestValidationRules = joi.object<ListRequest>({
    pageToken: joi.string(),
    numberOfResults: joi.number().integer().greater(0),
